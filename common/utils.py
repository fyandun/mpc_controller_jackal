...
import common.robot_motion as state
import common.quintic_polynomial_planner as qp
...
def mavg(array_in, window_size = 81): #remember to put an odd number here!
    ...
    for i in range(length):
        if (i < (window_size-1)/2):
            buffer = array_in[0:i+1+window_size/2]
            out_val = np.mean(buffer)
        elif (i > length-(window_size-1)/2):
            buffer = array_in[(i-window_size/2):]
            out_val = np.mean(buffer)
        else:
            buffer = array_in[(i-window_size/2):i+1+window_size/2]
            out_val = np.mean(buffer)
